...
export default function NavBar() {
  ...
  return (
    <>
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, justifySelf: "start", minWidth: 0 }}>
              ...
              <div style={{ minWidth: 0 }}>
                <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: c.text, lineHeight: 1.2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                  MRSPTU Bathinda
                </p>
            ...
            {/* Center pills */}
            <div className="navbar-center-pills" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}>
              {cgpa && (
                <div style={{ display: "flex", alignItems: "center", gap: 8, background: dark ? "rgba(129,140,248,0.12)" : "rgba(109,40,217,0.07)", border: `1px solid ${dark ? "rgba(129,140,248,0.25)" : "rgba(109,40,217,0.15)"}`, borderRadius: 99, padding: "5px 14px" }}>
                  <div style={{ width: 6, height: 6, borderRadius: "50%", background: scoreClr(cgpa), flexShrink: 0 }} />
                  <span style={{ fontSize: 11, color: c.sub, fontWeight: 500 }}>CGPA</span>
                  <span style={{ fontSize: 16, fontWeight: 800, color: scoreClr(cgpa), letterSpacing: -0.3 }}>{cgpa}</span>
                  <span style={{ fontSize: 10, color: c.muted, borderLeft: `1px solid ${c.border}`, paddingLeft: 8, marginLeft: 2 }}>
                    {(parseFloat(cgpa) * 10).toFixed(1)}%
                  </span>
                </div>
              )}
              {saveMsg && (
                <span style={{ fontSize: 11, color: c.ok, fontWeight: 600, background: dark ? "rgba(52,211,153,0.12)" : "rgba(5,150,105,0.08)", border: `1px solid ${dark ? "rgba(52,211,153,0.25)" : "rgba(5,150,105,0.2)"}`, borderRadius: 99, padding: "3px 12px", whiteSpace: "nowrap" }}>
                  ✓ {saveMsg}
                </span>
              )}
              {totalBacklogs > 0 && (
                <button onClick={handleBacklogTab} style={{ fontSize: 11, fontWeight: 600, color: c.bad, background: dark ? "rgba(248,113,113,0.12)" : "rgba(220,38,38,0.07)", border: `1px solid ${dark ? "rgba(248,113,113,0.25)" : "rgba(220,38,38,0.2)"}`, borderRadius: 99, padding: "3px 12px", cursor: "pointer", whiteSpace: "nowrap" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 8 }}>
              ...
              <div className="navbar-desktop-user" style={{ position: "relative" }}>
                <button onClick={handleToggleAvatar} style={{ display: "flex", alignItems: "center", gap: 8, background: c.hover, border: `1px solid ${showAvatarMenu ? c.accent : c.border}`, borderRadius: 99, padding: "4px 12px 4px 4px", cursor: "pointer", fontFamily: "inherit", transition: "all 0.2s" }}>
                  ...
                  <span className="navbar-username-text" style={{ fontSize: 12, color: c.text, fontWeight: 500, maxWidth: 90, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {user?.username}
                  </span>
                ...
                {showAvatarMenu && (
                  <>
                    <div onClick={handleCloseAvatar} style={{ position: "fixed", inset: 0, zIndex: 148 }} />
                    <div style={{ position: "absolute", top: "calc(100% + 8px)", right: 0, zIndex: 149, background: dark ? "#0f1424" : "#fff", border: `1px solid ${c.border}`, borderRadius: 16, boxShadow: dark ? "0 16px 48px rgba(0,0,0,0.5)" : "0 16px 48px rgba(109,40,217,0.12)", minWidth: 260, overflow: "hidden", animation: "scaleIn 0.15s ease both" }}>

                      {/* Header */}
                      <div style={{ padding: "16px", borderBottom: `1px solid ${c.border}`, background: dark ? "rgba(255,255,255,0.02)" : "rgba(109,40,217,0.02)" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                          <div style={{ width: 44, height: 44, borderRadius: "50%", background: "linear-gradient(135deg,#6d28d9,#a78bfa)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17, fontWeight: 900, color: "#fff", flexShrink: 0, boxShadow: "0 4px 12px rgba(109,40,217,0.35)" }}>
                            {initials}
                          </div>
                          <div style={{ minWidth: 0, flex: 1 }}>
                            <p style={{ margin: 0, fontSize: 14, fontWeight: 800, color: c.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{user?.username}</p>
                            <p style={{ margin: 0, fontSize: 11, color: c.sub }}>{user?.email}</p>
                          </div>
                        </div>
                        {cgpa && (
                          <div style={{ marginTop: 10, padding: "8px 10px", borderRadius: 8, background: dark ? "rgba(45,212,170,0.08)" : "rgba(5,150,105,0.06)", border: `1px solid ${dark ? "rgba(45,212,170,0.2)" : "rgba(5,150,105,0.15)"}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                            <span style={{ fontSize: 11, color: c.muted }}>Current CGPA</span>
                            <span style={{ fontSize: 18, fontWeight: 800, color: scoreClr(cgpa) }}>{cgpa}</span>
                          </div>
                        )}
                      </div>
                      ...
                      <div style={{ padding: "6px" }}>
                        ...
                        >
                          ...
                          <div style={{ textAlign: "left" }}>
                            ...
                            {daysLeft > 0 && <p style={{ margin: 0, fontSize: 10, color: c.muted }}>Available in {daysLeft} day{daysLeft === 1 ? "" : "s"}</p>}
                          </div>
                        </button>
        ...
        {showSecondBar && (
          <div className="navbar-second-bar" style={{ background: dark ? "rgba(19,22,42,0.9)" : "rgba(248,247,255,0.9)", backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)", borderBottom: `1px solid ${dark ? "rgba(129,140,248,0.1)" : "rgba(109,40,217,0.08)"}`, position: "relative", zIndex: 1 }}>
            <div className="navbar-second-bar-inner" style={{ maxWidth: 1080, margin: "0 auto", display: "flex", alignItems: "stretch", padding: "0 1.25rem" }}>
                        ...
                        return (
                          ...
                          >
                            ...
                            <span style={{ flex: 1 }}>
                              ...
                              <span style={{ display: "block", fontSize: 11, color: c.muted }}>{b.name}</span>
                            </span>
                            {isActive && <span style={{ fontSize: 13, color: c.accent, fontWeight: 700 }}>✓</span>}
                          </button>
                        );
                      })}
              ...
              {/* Tab strip */}
              <div className="no-scrollbar navbar-tabs-scroll" style={{ display: "flex", alignItems: "stretch", overflowX: "auto", flex: 1, justifyContent: "space-evenly" }}>
                {TABS.map(t => {
      ...
      {showProfileSheet && (
        <>
          <div onClick={handleCloseSheet} style={{ position: "fixed", inset: 0, zIndex: 998, background: "rgba(0,0,0,0.5)", backdropFilter: "blur(4px)" }} />
          <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 999, background: dark ? "#0f1424" : "#fff", borderRadius: "20px 20px 0 0", border: `1px solid ${c.border}`, borderBottom: "none", maxHeight: "85vh", overflowY: "auto", scrollbarWidth: "none", boxShadow: dark ? "0 -16px 48px rgba(0,0,0,0.55)" : "0 -16px 48px rgba(109,40,217,0.12)", animation: "slideUp 0.3s ease both", paddingBottom: "calc(env(safe-area-inset-bottom) + 24px)" }}>
            <style>{`@keyframes slideUp { from { transform: translateY(100%); } to { transform: translateY(0); } }`}</style>

            <div style={{ width: 40, height: 4, borderRadius: 99, background: dark ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.1)", margin: "10px auto 0" }} />

            {/* Header */}
            <div style={{ padding: "16px 20px", borderBottom: `1px solid ${c.border}` }}>
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                <div style={{ width: 52, height: 52, borderRadius: "50%", background: "linear-gradient(135deg,#6d28d9,#a78bfa)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, fontWeight: 900, color: "#fff", flexShrink: 0, boxShadow: "0 6px 16px rgba(109,40,217,0.4)" }}>
                  {initials}
                </div>
                <div style={{ minWidth: 0, flex: 1 }}>
                  <p style={{ margin: 0, fontSize: 17, fontWeight: 800, color: c.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{user?.username}</p>
                  <p style={{ margin: 0, fontSize: 11, color: c.sub }}>{user?.email}</p>
                </div>
                {cgpa && (
                  <div style={{ textAlign: "right", flexShrink: 0 }}>
                    <p style={{ margin: 0, fontSize: 9, color: c.muted, textTransform: "uppercase" }}>CGPA</p>
                    <p style={{ margin: 0, fontSize: 22, fontWeight: 900, color: scoreClr(cgpa) }}>{cgpa}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div style={{ padding: "8px 12px" }}>
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ fontSize: 16 }}>✏️</span>
                  <div style={{ textAlign: "left" }}>
                    <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: c.text }}>Change Username</p>
                    {daysLeft > 0 && <p style={{ margin: 0, fontSize: 10, color: c.muted }}>Available in {daysLeft} day{daysLeft === 1 ? "" : "s"}</p>}
                  </div>
                </div>
              ...
              {/* Install app */}
              <div style={{ padding: "13px 14px", borderRadius: 10, border: `1px solid ${c.border}`, background: c.hover, marginBottom: 4, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ fontSize: 16 }}>📲</span>
                  <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: c.text }}>Install App</p>
                </div>
