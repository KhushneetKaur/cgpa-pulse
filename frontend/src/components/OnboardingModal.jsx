...
function isValidUsername(u) {
  ...
  if (!/^[a-zA-Z0-9_]+$/.test(v)) return { ok: false, msg: "Only letters, numbers, underscores" };
...
export default function OnboardingModal({ user, onDone }) {
  ...
  const handleBranchNext = useCallback(async () => {
    ...
    try {
      ...
      setErr(err.message || "Failed to save branch. Try again.");
    } finally {
      setLoading(false);
    }
  }, [branch]);
      ...
      <div style={{
                ...
                return (
                  ...
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}>
                      <span style={{ width: 8, height: 8, borderRadius: "50%", background: b.color, flexShrink: 0, boxShadow: isSelected ? `0 0 6px ${b.color}` : "none" }} />
                      <span style={{ fontSize: 12, fontWeight: isSelected ? 700 : 600, color: isSelected ? b.color : c.text }}>
                        {b.short}
                      </span>
                    </div>
        ...
        {step === 4 && (
          <form onSubmit={handleFinish}>
            ...
            <div style={{ background: dark ? "#080c18" : "#f4f3ff", borderRadius: 12, padding: "16px", marginBottom: 20 }}>
              <p style={{ margin: "0 0 12px", fontSize: 11, fontWeight: 700, color: c.muted, textTransform: "uppercase", letterSpacing: 0.8 }}>
                Quick guide
              </p>
              {TIPS.map(tip => (
                <div key={tip.title} style={{
                  ...
                  background:   tip.highlight ? (dark ? "rgba(124,131,245,0.12)" : "rgba(109,40,217,0.07)") : "transparent",
                  border:       tip.highlight ? `1px solid ${dark ? "rgba(124,131,245,0.25)" : "rgba(109,40,217,0.18)"}` : "none",
                }}>
                  <span style={{ fontSize: 20, flexShrink: 0, lineHeight: 1.3 }}>{tip.icon}</span>
                  <div>
