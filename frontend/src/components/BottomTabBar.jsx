    ...
    <>
        ...
        {MORE_TABS.map(t => {
          ...
          return (
            ...
            >
              ...
              <span style={{ flex: 1, fontSize: 14, fontWeight: isActive ? 700 : 400, color: isActive ? c.accent : c.text }}>
                {t.label}
              </span>
              {showBadge && (
                <span style={{ background: c.bad, color: "#fff", fontSize: 10, fontWeight: 700, borderRadius: 99, padding: "2px 7px" }}>
                  {totalBacklogs}
                </span>
              )}
              {isActive && <span style={{ fontSize: 13, color: c.accent }}>✓</span>}
            </button>
          );
        })}
      </div>
      ...
      <nav
        ...
        style={{
          ...
          display:              "flex",
          alignItems:           "stretch",
          background:           dark ? "rgba(8,12,24,0.97)" : "rgba(255,255,255,0.97)",
          backdropFilter:       "blur(20px)",
          WebkitBackdropFilter: "blur(20px)",
          borderTop:            `1px solid ${dark ? "rgba(129,140,248,0.15)" : "rgba(109,40,217,0.1)"}`,
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            fontFamily:     "inherit",
            position:       "relative",
            borderTop:      (showMore || isMoreActive) ? `2px solid ${c.accent}` : "2px solid transparent",
            transition:     "all 0.15s",
          }}
        >
          {isMoreActive && !showMore && (
            <span style={{ position: "absolute", top: 6, right: "calc(50% - 14px)", width: 6, height: 6, borderRadius: "50%", background: c.accent }} />
          )}

          <div style={{ display: "flex", flexDirection: "column", gap: 4, alignItems: "center", justifyContent: "center", width: 20, height: 20, position: "relative" }}>
            {showMore ? (
              <span style={{ fontSize: 18, color: c.accent, lineHeight: 1, fontWeight: 700 }}>✕</span>
            ) : (
              HAMBURGER_BARS.map(i => (
                <div key={i} style={{ width: i === 1 ? 12 : 18, height: 2, borderRadius: 99, background: isMoreActive ? c.accent : c.muted, transition: "all 0.2s" }} />
              ))
            )}
          </div>

          <span style={{ fontSize: 9, fontWeight: (showMore || isMoreActive) ? 700 : 400, color: (showMore || isMoreActive) ? c.accent : c.muted, letterSpacing: 0.3, textTransform: "uppercase" }}>
            More
          </span>
        </button>
      </nav>
