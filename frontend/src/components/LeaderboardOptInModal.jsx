...
const LeaderboardOptInModal = memo(function LeaderboardOptInModal({ onConfirm, onCancel }) {
  const { c, dark, btn } = useTheme();
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p style={{ margin: "0 0 20px", fontSize: 13, color: c.sub }}>
          Your CGPA will be visible to all CGPA Pulse users.
        </p>

        <div style={{
          padding:      "14px 16px",
          background:   dark ? "rgba(148,163,184,0.08)" : "rgba(217,119,6,0.05)",
          border:       `1px solid ${dark ? "rgba(148,163,184,0.2)" : "rgba(217,119,6,0.15)"}`,
          borderRadius: 10,
          marginBottom: 20,
        }}>
          <p style={{ margin: "0 0 10px", fontSize: 13, fontWeight: 700, color: dark ? "#94a3b8" : "#92400e" }}>
          ...
          <ul style={{ margin: 0, paddingLeft: 18, fontSize: 13, color: c.sub, lineHeight: 1.8 }}>
            <li>
              ...
              <strong style={{ color: c.text }}>CGPA</strong> will be visible to all students using CGPA Pulse.
            </li>
            <li>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={onCancel}  style={{ ...btn("ghost"),   flex: 1 }}>
            Cancel
          </button>
          <button onClick={onConfirm} style={{ ...btn("primary"), flex: 2, padding: "10px 16px" }}>
            ✓ I understand — Opt In
          </button>
        </div>
      </div>
...
export default LeaderboardOptInModal;