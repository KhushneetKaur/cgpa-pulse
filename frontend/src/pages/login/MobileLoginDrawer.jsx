...
// Precompute colors into CARDS — gradeClr never called at render time
const CARDS = [
...
const LINES = [
  ...
  { text: "> Loading academic branches...        [OK]", delay: 1000, color: "#2dd4aa" },
...
const TerminalChrome = (
  <div style={{
    ...
    <span style={{ fontSize: 11, color: "rgba(255,255,255,0.3)", marginLeft: 8, fontFamily: "monospace" }}>
      cgpa_pulse.sh
    </span>
  </div>
);

// ── FloatingCards — memoized, uses precomputed colors ────────────────────────
const FloatingCards = memo(function FloatingCards({ dark }) {
  return (
      ...
      {CARDS.map((card, i) => (
        <div key={i} style={{
          ...
          <div style={{
            padding:        "8px 11px",
            borderRadius:   10,
            background:     dark ? "rgba(15,20,36,0.88)" : "rgba(255,255,255,0.88)",
            ...
            <div style={{ display: "flex", alignItems: "baseline", gap: 5 }}>
              <span style={{ fontSize: 17, fontWeight: 800, color: card.color }}>{card.total}</span>
              <span style={{ fontSize: 12, fontWeight: 700, color: card.color }}>{card.grade}</span>
            </div>
          </div>
        </div>
      ))}
...
// ── TypingText — unchanged logic, unchanged behavior ─────────────────────────
const TypingText = memo(function TypingText({ dark }) {
  ...
  return (
    <span style={{ color: dark ? "#a78bfa" : "#7c3aed", fontWeight: 700 }}>
      {displayed}
      <span style={{
...
// ── Terminal — memoized ───────────────────────────────────────────────────────
const Terminal = memo(function Terminal({ onDone, onOpenAbout }) {
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, overflow: "hidden" }}>
          ...
          <div style={{ padding: "16px 18px", minHeight: 200 }}>
            {visible.map((line, i) => {
                  ...
                  <div key={i} style={{ margin: "12px 0" }}>
                    <button
                      ...
                      style={{
                        ...
                        padding: "10px 16px", color: "#10b981",
                        fontSize: "clamp(11px,2vw,14px)", fontFamily: "monospace",
                    ...
                    >
                      ...
                      <span>
                        [ Developer Console ]
                        <span style={{ color: "rgba(16,185,129,0.55)", marginLeft: 8, fontSize: "clamp(9px,1.5vw,11px)" }}>
              ...
              return (
                <p key={i} style={{
                  margin: "2px 0", fontSize: "clamp(11px,1.8vw,13px)",
                  ...
                  {i === visible.length - 1 && !allDone && (
                    <span style={{
                      ...
                      background: "#7c83f5", marginLeft: 2,
                      verticalAlign: "middle", animation: "blink 0.7s step-end infinite",
                    }} />
                  )}
                </p>
              );
        ...
        {allDone && (
          <p style={{ textAlign: "center", marginTop: 16, fontSize: 11, color: "rgba(255,255,255,0.25)", animation: "blink 1.5s ease-in-out infinite" }}>
...
export default function MobileLoginDrawer({ handleGoogleLogin, dark, onOpenAbout }) {
  ...
  const openAbout = useCallback(() => {
    setShowAbout(true);
    onOpenAbout?.(); // optional chaining replaces if check
  }, [onOpenAbout]);
  ...
  return (
    <>
      {/* Terminal phase */}
      {phase === "terminal" && (
        <div style={{ opacity: termFading ? 0 : 1, transition: "opacity 0.4s ease" }}>
      ...
      {phase === "main" && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: "55%", pointerEvents: "none", zIndex: 1,
              background: dark ? "linear-gradient(180deg,rgba(8,12,24,0.9) 0%,transparent 100%)" : "linear-gradient(180deg,rgba(244,243,255,0.9) 0%,transparent 100%)" }} />
            <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: "65%", pointerEvents: "none", zIndex: 1,
              background: dark ? "linear-gradient(0deg,rgba(8,12,24,0.97) 0%,transparent 100%)" : "linear-gradient(0deg,rgba(244,243,255,0.97) 0%,transparent 100%)" }} />
          </div>
          ...
          <div style={{
            ...
            <p style={{ margin: "0 0 8px", fontSize: "clamp(10px,1.4vw,13px)", color: dark ? "rgba(255,255,255,0.35)" : "rgba(30,27,75,0.4)", letterSpacing: 1.5, textTransform: "uppercase", fontWeight: 600 }}>
              MRSPTU Bathinda
            </p>
            <h1 style={{ margin: 0, fontSize: "clamp(52px,10vw,108px)", fontWeight: 900, letterSpacing: "clamp(-2px,-0.4vw,-5px)", lineHeight: 1, color: dark ? "rgba(255,255,255,0.93)" : "#1e1b4b" }}>
              CGPA
            </h1>
            <h1 style={{
              margin: "0 0 clamp(10px,2vw,18px)", fontSize: "clamp(52px,10vw,108px)",
              fontWeight: 900, fontStyle: "italic", letterSpacing: "clamp(-2px,-0.4vw,-5px)",
              lineHeight: 1.1, paddingBottom: 4, paddingRight: 18,
              backgroundImage: dark ? "linear-gradient(135deg,#c084fc 0%,#67e8f9 100%)" : "linear-gradient(135deg,#7c3aed 0%,#10b981 100%)",
            ...
            <p style={{ margin: 0, fontSize: "clamp(13px,1.8vw,18px)", color: dark ? "rgba(255,255,255,0.5)" : "#5b5687", lineHeight: 1.5, pointerEvents: "auto" }}>
          ...
          <div style={{
            ...
            {!isDesktop && (
              <div style={{ width: 40, height: 4, borderRadius: 99, background: dark ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.1)", margin: "0 auto clamp(14px,2.5vw,22px)" }} />
            )}

            <p style={{ margin: "0 0 4px", fontSize: "clamp(17px,2.5vw,22px)", fontWeight: 800, color: dark ? "rgba(255,255,255,0.92)" : "#1e1b4b", textAlign: "center" }}>
              Sign in to get started
            </p>
            <p style={{ margin: "0 0 clamp(14px,2vw,20px)", fontSize: "clamp(11px,1.4vw,13px)", color: dark ? "rgba(255,255,255,0.38)" : "#a09bbf", textAlign: "center" }}>
            ...
            <button
              ...
              style={{
                width: "100%", padding: "clamp(12px,1.8vw,16px) 16px",
                borderRadius: 14, border: `1.5px solid ${dark ? "rgba(255,255,255,0.14)" : "#dadce0"}`,
                background: dark ? "rgba(255,255,255,0.08)" : "#fff",
                color: dark ? "rgba(255,255,255,0.9)" : "#3c4043",
                fontSize: "clamp(14px,1.6vw,16px)", fontWeight: 600, fontFamily: "inherit",
            ...
            <button
              onClick={openAbout}
              style={{
                width: "100%", padding: "clamp(10px,1.5vw,13px) 14px",
                borderRadius: 12,
                background: dark ? "rgba(16,185,129,0.06)" : "rgba(16,185,129,0.04)",
                border: "1px solid rgba(16,185,129,0.25)",
                cursor: "pointer", fontFamily: "inherit",
                display: "flex", alignItems: "center", gap: 12, textAlign: "left",
                transition: "all 0.2s", animation: "termGlow 3s ease-in-out infinite",
            ...
            >
              <div style={{
                ...
                borderRadius: "50%", background: "linear-gradient(135deg,#6d28d9,#a78bfa)",
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: "clamp(11px,1.6vw,14px)", fontWeight: 900, color: "#fff",
                flexShrink: 0, boxShadow: "0 0 12px rgba(109,40,217,0.4)",
              }}>
                KK
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ margin: "0 0 1px", fontSize: "clamp(12px,1.5vw,14px)", fontWeight: 700, color: dark ? "rgba(255,255,255,0.88)" : "#1e1b4b" }}>
                  Khushneet Kaur
                </p>
                <p style={{ margin: 0, fontSize: "clamp(9px,1.2vw,11px)", color: dark ? "rgba(255,255,255,0.5)" : "#7b6fa0" }}>
                  Developer
                </p>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 4, background: "rgba(16,185,129,0.12)", border: "1px solid rgba(16,185,129,0.35)", borderRadius: 6, padding: "4px 8px", flexShrink: 0 }}>
                ...
                <span style={{ fontSize: 9, fontWeight: 700, color: "#10b981", fontFamily: "monospace", letterSpacing: 0.4 }}>
                  CONSOLE
                </span>
              </div>
            </button>

            <p style={{ textAlign: "center", fontSize: "clamp(10px,1.3vw,12px)", color: dark ? "rgba(255,255,255,0.45)" : "#7b6fa0", lineHeight: 1.6, margin: 0 }}>
      ...
      {showAbout && (
        <AboutModal onClose={closeAbout} dark={dark} />
      )}
    </>
  );
}