...
// Sync leaderboard after branch change — always fires, no opt-in gate
async function syncLeaderboard(user, userId) {
  ...
  if (semCount === 0) return; // no ghost entries
...
export async function updateUsername(req, res, next) {
  try {
    ...
    await user.save();
    await syncLeaderboard(user.toObject(), userId);
...
export async function checkUsername(req, res, next) {
  try {
    ...
    const existing = await User.findOne({
      username: { $regex: new RegExp(`^${base}$`, "i") },
    });
...
export async function updateBranch(req, res, next) {
  try {
    ...
    // Infer faculty from branch — keep frontend and backend in sync
    const PHARMACY_CODES  = ["BPHARM", "PHARMD", "MPHARM"];
    const faculty         = PHARMACY_CODES.includes(branch?.toUpperCase())
      ? "pharmacy"
      : "engineering";
    ...
    await syncLeaderboard(user.toObject(), userId);
    sendResponse(res, 200, { branch: user.branch, faculty: user.faculty }, "Branch updated");
  } catch (err) { next(err); }
}

// ── PUT /api/user/faculty ─────────────────────────────────────────────────────
export async function updateFaculty(req, res, next) {
  try {
    ...
    sendResponse(res, 200, { user: user.toPublicJSON() }, "Faculty updated");
  } catch (err) { next(err); }
}
